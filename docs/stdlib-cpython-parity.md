...
| `socket` | `Socket` class with `__init__(family, type)`, `connect((host, port))`, `send`, `recv`, `close`; `AF_INET`, `SOCK_STREAM` | `bind` / `listen` / `accept` / `shutdown` / `setsockopt` / `getsockopt` / `recvfrom` / `sendto` / `sendall` / `makefile` / `fromfd` / `socketpair`, `gethostname` / `gethostbyname` / `getaddrinfo(host, port, family=0, type=0, proto=0, flags=0)`, IPv6 (`AF_INET6`), Unix (`AF_UNIX`), UDP (`SOCK_DGRAM`), `settimeout`, `setblocking`, the dozens of `SOL_*` / `SO_*` / `IPPROTO_*` constants, `socket.error` / `herror` / `gaierror` / `timeout`. Documented as "v1, TCP client only." |
